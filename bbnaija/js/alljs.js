...
var reposted=Number( v.repost);
 ...
 var fullname_= veri.name;
  ...
  var can_comment= +(data.can_comment||0);
  var shareable        = +(data.shareable||0)
  var hidden_post= +(data.hidden||0);
  var commentable = +(data.commentable||0);
  var has_more=+(data["hmore"]||0);
...
if(  spresult){ 
    ...
    oshareable=+(odata.shareable||0);
    ohidden=+(odata.hidden||0);
...
function checkUrl( href){
   var site=__SITE_URL__.replace("www.","");
   var href_=href.replace("www.","");
  
if( !href_.match( new RegExp("\\b" + site , "gi") ) ){
	return window.open(href, '_blank').focus(); 
	}

	href=href.split("?")[0];
	var params=href.replace("www.","").replace(new RegExp("\\b" + site + "/?"),"");
